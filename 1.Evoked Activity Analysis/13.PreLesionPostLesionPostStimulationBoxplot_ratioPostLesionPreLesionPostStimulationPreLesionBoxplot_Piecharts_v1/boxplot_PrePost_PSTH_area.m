...
%------
    ...
    for n=1:nareas_x_nmapping
        ...
        boxplot([datatoplot{n,1},datatoplot{n,2}],['PreL','PosL',strcat("PoS (",datatoplot{n,5},')')])
        xticklabels({})
    ...
    xticklabels(nexttile(3),['PreL','PosL',strcat("PoS (",datatoplot{n,5},')')])
    xticklabels(nexttile(4),['PreL','PosL',strcat("PoS (",datatoplot{n,5},')')])
...
%-------------------------------
    ...
    for n=1:nareas_x_nmapping
        meanPreL=mean(datatoplot{n,1}(~isnan(datatoplot{n,1}(:,1)),1));

        currentTile = nexttile(n);

        boxplot([datatoplot{n,1}(:,2)/meanPreL,datatoplot{n,2}/meanPreL],['PosL',strcat("PoS (",datatoplot{n,5},')')])
        xticklabels({})
        hold on
        yline(1,'--')
...
%--------------
    ...
    subp_pie = tiledlayout(ceil(sqrt(nareas_x_nmapping)),ceil(sqrt(nareas_x_nmapping)),"TileSpacing","tight","Padding","compact");

    meanPreL=mean(datatoplot{n,1}(~isnan(datatoplot{n,1}(:,1)),1));
    all_phases=[datatoplot{n,1}(:,2),datatoplot{n,2}];
    all_phases_names=['PosL',strcat("PoS (",datatoplot{n,5},')')];

    for npie=1:4

        currentTile = nexttile(npie);
        
        greater=sum(all_phases(~isnan(all_phases(:,npie)),npie)>meanPreL)/sum(~isnan(all_phases(:,npie)));

        pie([greater,1-greater]);

        title(all_phases_names(npie))

    end 

    nexttile(ceil(sqrt(nareas_x_nmapping)));
    legend({'> 1','< 1'})
    legend box off
