% tutti et1-ol con tutti sham-ol confrontando il prelesione con il post
% lesione. In questo modo non entra in gioco l'effetto della stimolazione e
% si può studiare l'effetto della lesione. solo et1-ol e sham-ol perchè
% negli altri gruppi non c'è il psth post-lesione
...
LesionType_PrePost_PSTHareagraph_scatter_directory=uigetdir(fullfile(PrePost_PSTHarea_directory,'\..'),'Select Directory Where To Save (la cartella in cui salvare gli scatterplot delle aree dei PSTH per tipo di lesione)');
...
for l=1:2
            ...
            for h=1:nmaptypes
                for t=1:2
                    ...
                    for s=1:nstimchs
                        ...
                        if length(datatoplot{row,3})~=nphasemax && length(datatoplot{row,3})<length(mapphases)
    ...
    for n=1:nareas_x_nmapping
        ...
        datatoplot{n,1}(:,3:end)=[];
        datatoplot{n,3}(3:end,:)=[];
    ...
    title(subp_scatter,strcat(lesion))
    xlabel(subp_scatter,'PSTH areas Pre')
    ylabel(subp_scatter,'PSTH areas Post')
    exportgraphics(groups_scatter_PSTHarea_figure,fullfile(LesionType_PrePost_PSTHareagraph_scatter_directory,strcat('PSTHareas_',lesion,".png")),'Resolution',500)
    groups_scatter_PSTHarea_figure.Visible="on";
    saveas(groups_scatter_PSTHarea_figure,fullfile(LesionType_PrePost_PSTHareagraph_scatter_directory,strcat('PSTHareas_',lesion,".fig")))
