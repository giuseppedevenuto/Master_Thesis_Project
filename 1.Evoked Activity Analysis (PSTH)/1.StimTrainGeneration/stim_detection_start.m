function [single_stim_train,sorted_true_active_ch,sorted_n_stim] = stim_detection_start(pulse_train,idx_active_channels,n_stim_ch)
% crea il treno di stimolazioni quando lo stimolo inizia, cioè passa da un
% valore nullo ad un valore negativo
...
for ch=1:length(active_ch)
    ...
    idx_stim = (([diff(pulse_train(active_ch(ch),:)),0]+pulse_train(active_ch(ch),:))<0).*[(diff(pulse_train(active_ch(ch),:))<0),0];
    stim_train(ch,:) = full(idx_stim); % converte in double
