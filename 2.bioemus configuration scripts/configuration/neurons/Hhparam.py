...
NB_HHPARAM   = 16
SCALE_FACTOR = 256 # <EDITABLE> termine di amplificazione per avere l'effetto di una rete più grande
...
class Hhparam:
    ...
    def getParameters(self, nrn_type:str, dt, config_param = pd.DataFrame()):
        ...
        if nt[0] == "FS":
            ...
            if not("nostim" in nt):
                hhparam[self.PID["i_stim"]]  = 0.01 # 0.0075 # 0.03          # (mA/cm²)
            ...
            hhparam[self.PID["G_Na"]]        = 50e-3         # (S/cm²)
            hhparam[self.PID["G_Kd"]]        =  5e-3         # (S/cm²)
            hhparam[self.PID["G_M"]]         = 0.070e-3      # (S/cm²)
            hhparam[self.PID["G_L"]]         = 0.0           # (S/cm²)
            hhparam[self.PID["G_T"]]         = 0.0           # (S/cm²)
            ...
            hhparam[self.PID["E_Leak"]]      = -70.0         # (mV)
            if not("nonoise" in nt):
                hhparam[self.PID["mu"]]      = 0.03         # 0.05
                hhparam[self.PID["theta"]]   = 6.5           # 8.0
                hhparam[self.PID["sigma"]]   = 0.075         # 0.1
            if not("nostim" in nt):
            ...
            hhparam[self.PID["E_Leak"]]      = -70.0         # (mV)
            if not("nonoise" in nt):
                hhparam[self.PID["mu"]]      = 0.025         # 0.1
                hhparam[self.PID["theta"]]   = 6.2           # 8.0
                hhparam[self.PID["sigma"]]   = 0.060         # 0.12
            if not("nostim" in nt):
                hhparam[self.PID["i_stim"]]  = 0.03         # (mA/cm²)
            hhparam[self.PID["v_init"]]      = -70.0         # (mV)
            ...
            if not("nonoise" in nt):
                if config_param.empty:
                    ...
                    muRS = float(config_param['μRS'].to_numpy()[0])
                # print(muRS)
                hhparam[self.PID["mu"]]      = muRS         # 0.1  <EDIT> injected noise
