...
from spkmon.ui.main_window_ui import Ui_MainWindow
from spkmon.settings.defaults  import *
from spkmon.settings.config    import *
from spkmon.maxwell.customStim import *
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.maxw_thread    = MaxwellThread()
    ...
    def update_raster(self, spk_tab):
        """Update raster and detect burst"""
        x = []
        y = []

        THRESH_NB_SPK      = 15 # Number of spikes to consider a burst on a neuron in one buffer (100*1ms)
        THRESH_NB_NEUR     = 64 # Number of neurons bursting to consider a burst
            ...
            for i in range(NB_SPK_DATA_PER_FRAME): # nb of regs
                ...
                for k in range(DATAWIDTH_BIT_FRAME): # nb of nrn per data
                    if reg & (1<<k) != 0:
                        x.append(tstamp)
                        y.append(nid)
                        spk_cnt[nid] += 1 # count spikes for each neurons
                    nid += 1
        self.scatter.addPoints(x, y)

        # Detect burst
        for nid in range(NB_NRN):
            # Check if a neuron is bursting
            if spk_cnt[nid] > THRESH_NB_SPK:
                burst_cnt += 1

        # Check how many neurons are bursting
        if burst_cnt > THRESH_NB_NEUR:
            self.maxw_thread.stim.emit(True) # Send stimulation maxwell
...
class MaxwellThread(QThread):
    stim  = pyqtSignal(bool)

    def __init__(self):
        """Initialize"""
        super().__init__()
        self.stim.connect(self.sendStimToMaxwell) # Connect stimulation signal to stimulation send on Maxwell
    
    def sendStimToMaxwell(stim_trig):
        sendMaxwellStim(stim_trig) # Trigger stimulation Maxwell
        
    def run(self):
        """Run serial port reading in a thread"""
        initMaxwellStim() # Init maxwell device
        while True:
            time.sleep(0.1)